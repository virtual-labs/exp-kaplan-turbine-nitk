...
function goToNextFunction()
{
	...
	else if(simsubscreennum == 5)
	{
		...
		if(lnt == 1)
		{
			...
			generateQuestion(q2,"Type of flow in Kaplan turbine: ","","Centrifugal flow","Radial flow","Tangential flow","Axial flow",4,screen5Proceed,410,140,310,160);
		}
		else if(lnt == 2)
		{
			var q3 = Object.create(questions);
			generateQuestion(q3,"Head of water (h) is calculated by: ","","h = Initial hook gauge reading - Final hook gauge reading","h = Final hook gauge reading -  Initial hook gauge reading","h = Final hook gauge reading +  Initial hook gauge reading","h = Final hook gauge reading *  Initial hook gauge reading",2,screen5Proceed,390,90,355,180);
		}
		else
		document.getElementById("nextButton").style.visibility = "visible";
	}
...
// 	if(ele.id == "can6-8")
// 		textDisplay = "Unit power (W) = Output Power / H<sup>3/2</sup>";
// 	if(ele.id == "can6-9")
...
function magic()
{
	...
	else if (simsubscreennum==3)
	{
		...
		else
		{
			...
			document.getElementById('can3-4').innerHTML="Weight on the pan (W) = ";
...
function step1andhalf()
{
	...
	setTimeout(function()
	{
		...
		generateQuestion(q1,"Kaplan turbine is also known as : ","","Turgo turbine","Pelton turbine","Propeller turbine","None of the above",3,scree1Proceed,370,350,310,170);
	},800);
...
function step2()
{
	...
	setTimeout(function(){
	document.getElementById('can2-4').innerHTML="Constant head (H) =" +y[x].text +" m ";
	document.getElementById('nextButton').style.visibility="visible";
	}, 1700);
}
...
function step3()
{
	...
	setTimeout(function(){
		document.getElementById('can3-4').innerHTML="Weight on the pan (W) = "+values[x][lnt][5] +" kg";
		document.getElementById('can3-5').innerHTML="Spring balance reading (S) = "+values[x][lnt][6] +" kg" ;
...
function step3andhalf()
{
	...
	setTimeout(function(){
		document.getElementById('can3-4').innerHTML="Dead weight of the pan (W) ="+values[x][lnt][5] +" kg"
		document.getElementById('can3-5').innerHTML="Spring balance reading (S) = "+values[x][lnt][6] +" kg" ;
		document.getElementById('can3-6').innerHTML="Torque (&tau;) = ";
		idInput = document.getElementById('can3-6');
		userCalculation(idInput);
...
function step5quarter()
{
	setTimeout(function(){
		...
		document.getElementById('can5-11').innerHTML="Head of water (h) = "+values[x][lnt][3] + " cm";
...
{
	document.getElementById("formula").style.visibility = "hidden";
	document.getElementById("can6-2").innerHTML = "Constant head (H) = "+y[x].text +"m" ;
	document.getElementById("can6-3").innerHTML ="Actual discharge, Q<sub>act</sub> ="+ values[x][lnt][4]+"m<sup>3</sup>/sec";
	document.getElementById("can6-4").innerHTML = "Speed of motor (N) = "+values[x][lnt][0] +"rpm" ;
	document.getElementById("can6-4a").innerHTML = "Torque (&tau;) = "+values[x][lnt][7]+"N-m";
	myStopFunction();
	if(lnt == 0)
	{
		var hval = y[x].text+"m";
		var q1 = Object.create(questions);
		generateQuestion(q1,"Constant Head (H) value set in the experiment is : ","","9.5m","2m","3.5m",hval,4,scree6Proceed,400,350,310,170);
	}
	else
	{
		setTimeout(function(){
